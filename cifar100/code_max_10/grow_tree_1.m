function labelsTransform = grow_tree_1(value, index, labelsTransform, new_labels)
k = 1;
        ...
        if((v-v2 > 0.1) || (index(2,i) == index(1,i)))
            ...
            if (v-v3 > 0.1)
                if (v-v2 < 0.05) % Go ahead merge
                    if (numel(lt_old(lt_old(:,2)==index(2,i),1))>1)
                        labelsTransform(end+1,:) = [new_labels(i), index(1,i)];
                        value(:,i) = [];
                        index(:,i) = [];
                        new_labels(i) = [];
                    ...
                    if (ismember(index(2,i),i_old) || numel(lt_old(lt_old(:,2)==index(2,i),1))>1)
                        labelsTransform(end+1,:) = [new_labels(i), index(1,i)];
                        value(:,i) = [];
                        index(:,i) = [];
                        new_labels(i) = [];
