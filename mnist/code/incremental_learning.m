...
top_node.filePath = fullfile('../output/tree_incr/', sprintf('%d',top_node.id));
...
for i = 1 : numel(top_node.children)
    ...
    if numel(labels) > numel(next_node.labelsTransform(:,1))
        ...
        next_node.filePath = fullfile('../output/tree_incr/', sprintf('%d',next_node.id));
        next_node.opts.expDir = next_node.filePath;
        next_node.opts.numEpochs = 50;
